...
import argparse
from datasets.make_dataloader_stage3 import make_stage3_testloader, get_parameter
...
if __name__ == "__main__":
    ...
    # 加载target domain的数据
    train_loader, train_loader_normal, val_loader, num_query, _, _, _ = make_stage3_testloader(cfg)

    # 加载source domain的数据，用于构建模型
    ...
    model.load_param(cfg.Stage3.TEST_WEIGHT) # stage3训练完之后的
    # model.load_param(cfg.TEST.WEIGHT)  # stage2训练完之后的

    if cfg.DATASETS.NAMES == 'VehicleID':
        for trial in range(10):
            train_loader, train_loader_normal, val_loader, num_query, num_classes, camera_num, view_num = make_stage3_testloader(cfg)
