...
def do_train_stage3(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.train()
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader_stage2):  # img 256,3,256,128
            optimizer.zero_grad()
            ...
            with amp.autocast(enabled=True):
                score, feat, image_features = model(x=img, label=target, cam_label=target_cam,
                                                    view_label=target_view)  # [cls_score, cls_score_proj], [img_feature_last, img_feature, img_feature_proj], img_feature_proj
                ...
                if dist.get_rank() == 0:
                    torch.save(model.state_dict(),
                               os.path.join(cfg.OUTPUT_DIR,
                                            cfg.DATASETS.NAMES + cfg.MODEL.NAME + '_stage2_{}.pth'.format(epoch)))
            else:
                torch.save(model.state_dict(),
                           os.path.join(cfg.OUTPUT_DIR,
                                        cfg.DATASETS.NAMES + cfg.MODEL.NAME + '_stage2_{}.pth'.format(epoch)))
